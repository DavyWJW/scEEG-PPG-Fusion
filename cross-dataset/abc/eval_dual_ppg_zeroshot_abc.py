...
# -*- coding: utf-8 -*-
"""
跨数据集验证脚本 - 双流PPG模型 (MESA → ABC)
用MESA训练的PPG+Unfiltered PPG Cross-Attention模型在ABC数据集上测试

使用方法:
    python cross_dataset_dualppg_abc.py \
        --model_path ./outputs/ppg_unfiltered_xxx/checkpoints/best_model.pth \
        --abc_data_dir ./abc_processed
"""
...
# ============================================================================
# 1. ABC PPG数据集 (用于跨数据集测试)
# ============================================================================

class ABCPPGTestDataset(Dataset):
    """
    ABC PPG测试数据集
    加载预处理好的ABC PPG数据用于跨数据集测试
    ...
    注意: ABC记录通常不足10小时，会自动padding
    """

    def __init__(self, data_path, min_windows=600, verbose=True):
        """
        Args:
            data_path: 数据目录，包含abc_ppg_with_labels.h5和abc_subject_index.h5
            min_windows: 最小windows数要求
            verbose: 是否打印详细信息
        """
    ...
    def _load_subjects(self):
        ...
        if self.verbose:
            ...
            print(f"\n✅ 加载完成:")
            print(f"   有效被试数: {len(self.subjects)} / {len(all_subjects)}")
            print(f"   最小windows要求: {self.min_windows}")
            print(f"   平均每被试windows: {avg_windows:.1f}")
            print(f"   总有效windows数: {total_windows:,}")
...
def cross_dataset_ppg_evaluation(model_path, abc_data_dir, output_dir, config):
    ...
    if 'model_state_dict' in checkpoint:
        ...
        if 'best_kappa' in checkpoint:
            print(f"   模型最佳验证Kappa (MESA): {checkpoint['best_kappa']:.4f}")
    ...
    with torch.no_grad():
        for batch_idx, (ppg, labels, subject_ids) in enumerate(tqdm(test_loader, desc="推理进度")):
            ppg = ppg.to(device)
    ...
    # ========== 5. 打印结果 ==========
    print(f"\n{'=' * 70}")
    print(f"跨数据集评估结果: MESA → ABC (双流PPG模型, 零样本)")
    print(f"{'=' * 70}")
...
def main():
    parser = argparse.ArgumentParser(description='Cross-Dataset Evaluation: MESA → ABC (Dual-Stream PPG)')
    parser.add_argument('--model_path', type=str, required=True,
                        help='MESA训练的双流PPG模型路径 (.pth文件)')
    parser.add_argument('--abc_data_dir', type=str, required=True,
                        help='ABC数据目录 (包含abc_ppg_with_labels.h5和abc_subject_index.h5)')
    ...
    print("=" * 70)
    print("🔄 跨数据集验证: MESA → ABC (双流PPG模型, 零样本)")
    print("=" * 70)
    print("\n验证MESA训练的PPG+Unfiltered PPG模型在ABC上的泛化能力")
    print("注意: 这是零样本测试，不进行任何微调")
