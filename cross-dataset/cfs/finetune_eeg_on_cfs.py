...
用法:
    python finetune_eeg_on_cfs.py \
        --cfs_data_dir ../../data/cfs_eeg_c3m2_data \
        --pretrained_path ./best_mesa_4class_corrected_model.pth \
...
class CFSEEGDataset(Dataset):
    """CFS EEG数据集 - epoch级别
    
    CFS EEG数据格式: 
    - 文件名: 800002.npz (被试ID)
    - 内容: x (EEG epochs), y (标签)
    - 标签: 0=Wake, 1=Light, 2=Deep, 3=REM
    """

    def __init__(self, npz_files, split='train', seed=42, train_ratio=0.8):
        """
        Args:
            npz_files: NPZ文件列表
        ...
        print(f"CFS EEG总被试数: {len(all_subjects)}")

        # 划分数据集: 80% train+val, 20% test
        train_val_subjects, test_subjects = train_test_split(
            all_subjects, test_size=1-train_ratio, random_state=seed
        )
        # 从train_val中划分20%作为验证集
        train_subjects, val_subjects = train_test_split(
            train_val_subjects, test_size=0.2, random_state=seed
        )
        ...
        print(f"  {split} set: {len(self.subjects)} 被试")
...
class EEGFineTuner:
    ...
    def __init__(self, config):
        ...
        self.output_dir = os.path.join(
            config['output_dir'],
            f"finetune_cfs_eeg_{config['strategy']}_{timestamp}"
        )
    ...
    def train_epoch(self, model, dataloader, optimizer, criterion):
        ...
        for batch_idx, (data, target, _) in enumerate(tqdm(dataloader, desc="Training", leave=False)):
    ...
    def evaluate(self, model, dataloader, criterion):
        ...
        with torch.no_grad():
            for data, target, subject_ids in tqdm(dataloader, desc="Evaluating", leave=False):
                data = data.to(self.device)
                target = target.to(self.device)
    ...
    def train(self):
        ...
        # 加载数据
        npz_files = sorted(glob.glob(os.path.join(self.config['cfs_data_dir'], '*.npz')))
        if len(npz_files) == 0:
            raise FileNotFoundError(f"未在 {self.config['cfs_data_dir']} 找到NPZ文件")

        print(f"\n找到 {len(npz_files)} 个NPZ文件")

        # 创建数据集
        train_dataset = CFSEEGDataset(npz_files, split='train', seed=self.config.get('seed', 42))
        val_dataset = CFSEEGDataset(npz_files, split='val', seed=self.config.get('seed', 42))
        test_dataset = CFSEEGDataset(npz_files, split='test', seed=self.config.get('seed', 42))
        ...
        plt.title(f'CFS EEG Confusion Matrix (Kappa: {test_results["kappa"]:.3f})')
...
def main():
    parser = argparse.ArgumentParser(description='微调MESA预训练EEG模型到CFS')

    # 数据路径
    parser.add_argument('--cfs_data_dir', type=str, 
                        default='../../data/cfs_eeg_c3m2_data',
                        help='CFS EEG NPZ文件目录')
    parser.add_argument('--pretrained_path', type=str, 
                        default='./best_mesa_4class_corrected_model.pth',
                        help='MESA预训练模型路径')
    ...
    # 其他
    parser.add_argument('--output_dir', type=str, default='./finetune_cfs_eeg_outputs')
    ...
    # 构建配置
    config = {
        'cfs_data_dir': args.cfs_data_dir,
