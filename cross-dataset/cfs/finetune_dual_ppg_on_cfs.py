...
用法:
    python finetune_dual_ppg_on_cfs.py --strategy full --lr 1e-5
    python finetune_dual_ppg_on_cfs.py --strategy head_only --lr 1e-4
"""
...
class CFSDualPPGDataset(Dataset):
    """CFS PPG数据集 - 用于PPGUnfilteredCrossAttention模型
    ...
    def __init__(self, ppg_file, index_file, split='train', seed=42,
        ...
        train_subjects, val_subjects = train_test_split(
            train_subjects, test_size=0.2, random_state=seed
        )
        ...
        # 更新subjects列表
        self.subjects = [s for s in self.subjects if s in self.subject_indices]
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.ppg_file, 'r') as f:
            ...
            labels = f['labels'][start_idx:start_idx + 1200]

        # 拼接成连续信号
        ppg = ppg_windows.reshape(-1).astype(np.float32)
...
class DualPPGFineTuner:
    ...
    def __init__(self, config):
        ...
        self.output_dir = os.path.join(
            config['output_dir'],
            f"finetune_dual_{config['strategy']}_{timestamp}"
        )
    ...
    def calculate_class_weights(self, train_dataset):
        ...
        all_labels = []
        for idx in range(min(len(train_dataset), 50)):
    ...
    def evaluate(self, model, dataloader, criterion):
        ...
        overall_kappa = cohen_kappa_score(all_labels, all_preds)
    ...
    def train(self):
        ...
        # 创建数据集
        train_dataset = CFSDualPPGDataset(
            self.config['cfs_ppg_file'],
            self.config['cfs_index_file'],
            split='train'
        )
        val_dataset = CFSDualPPGDataset(
            self.config['cfs_ppg_file'],
            self.config['cfs_index_file'],
            split='val'
        )
        test_dataset = CFSDualPPGDataset(
            self.config['cfs_ppg_file'],
            self.config['cfs_index_file'],
            split='test'
        )
        ...
        criterion = nn.CrossEntropyLoss(weight=class_weights, ignore_index=-1, reduction='none')
        ...
        for epoch in range(1, self.config['num_epochs'] + 1):
            ...
            if self.config['strategy'] == 'progressive':
                if epoch == self.config.get('unfreeze_tcn_epoch', 5):
                    print("\n🔓 解冻TCN层和融合层")
                    self.unfreeze_layer(model, 'tcnblock1')
                    self.unfreeze_layer(model, 'tcnblock2')
                    self.unfreeze_layer(model, 'fusion_blocks')
                    self.unfreeze_layer(model, 'adaptive_weight')
...
def main():
    parser = argparse.ArgumentParser(description='微调MESA预训练双流PPG模型到CFS')

    # 数据路径
    parser.add_argument('--cfs_ppg_file', type=str,
                        default='../../data/cfs_ppg_with_labels.h5')
    parser.add_argument('--cfs_index_file', type=str,
                        default='../../data/cfs_subject_index.h5')
    parser.add_argument('--pretrained_path', type=str,
                        default='./outputs/dual_ppg_xxx/checkpoints/best_model.pth',
                        help='MESA预训练双流模型路径')
    ...
    # 构建配置
    config = {
        'cfs_ppg_file': args.cfs_ppg_file,
        'cfs_index_file': args.cfs_index_file,
    ...
    print("\n" + "=" * 70)
    print("MESA → CFS 双流PPG微调 (PPGUnfilteredCrossAttention)")
    print("=" * 70)
    print(f"\n配置:")
    ...
    print(
        f"  梯度累积: {config['gradient_accumulation_steps']} (有效batch={config['batch_size'] * config['gradient_accumulation_steps']})")
    print(f"  混合精度: {'启用' if config['use_amp'] else '禁用'}")
    print(f"  预训练模型: {config['pretrained_path']}")
    print(f"  CFS数据: {config['cfs_ppg_file']}")
